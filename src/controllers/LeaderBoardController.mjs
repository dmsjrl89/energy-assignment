...
//Handle post
  static handlePostLeaderBoard(req, res) {
    try {
      ...
      console.log("Received location data for leaderboard:", locationData);

      // 데이터를 성공적으로 처리했다면 클라이언트에 응답
      res.status(200).json({ message: "Location data successfully posted to the leaderboard", location: locationData });
    } catch (error) {
