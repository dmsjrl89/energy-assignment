  ...
  getTotalRunningHours() {
    let totalSum = 0
      for (const appliance of this.appliances) {
        ...
        const runningHours = Number(appliance.hours) || 0
        const count = Number(appliance.quantity) || 0
...
// static getWattsByName(applianceName) {
//   const appliance = this.appliances.find(a => a.name === applianceName);
//   return appliance ? appliance.watts : 0;  // Default to 0 if not found
